...
//                             <p className="text-sm sm:text-base text-gray-600">Scratch both cards to reveal your points! You get 2 cards daily.</p>
//                           </div>
//                           <div className="bg-gradient-to-r from-purple-100 to-pink-100 rounded-lg p-3 border-2 border-purple-300 shadow-sm">
...
//                   <div className="text-center">
//                     <p className="text-xs font-semibold text-pink-900 mb-1">⭐ Master</p>
//                     <p className="text-sm font-bold text-pink-900">450+ pts</p>
//                   </div>
//                 </div>
//               </div>
//             </div>
...
const Scard = () => {
  ...
  const NEW_YEAR_DATE = new Date("2026-01-01T00:00:00");
  ...
  useEffect(() => {
    const updateNewYearTimer = () => {
      ...
      if (diff <= 0) {
        setNewYearCountdown("🎉 HAPPY NEW YEAR 2026 🎉");
        return;
      }
      ...
      setNewYearCountdown(`${days}d ${hours}h ${minutes}m ${seconds}s`);
    };

    updateNewYearTimer();
    const interval = setInterval(updateNewYearTimer, 1000);
    return () => clearInterval(interval);
  }, []);
  ...
  useEffect(() => {
    ...
    updateDailyReset();
    const interval = setInterval(updateDailyReset, 1000);
    return () => clearInterval(interval);
  }, []);
  ...
  const fetchLeaderboard = async () => {
    try {
      const [lifetime, monthly, weekly] = await Promise.all([
        API.get("/scard/leaderboard?type=lifetime"),
        API.get("/scard/leaderboard?type=monthly"),
        API.get("/scard/leaderboard?type=weekly")
      ]);
      ...
      setLeaderboard({
        lifetime: lifetime.data.leaderboard || [],
        monthly: monthly.data.leaderboard || [],
  ...
  const handleScratchCard = async (index) => {
    try {
      const res = await API.post("/scard/scratch", { cardIndex: index });
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <p className="text-2xl text-green-700">Loading your game...</p>
      </div>
    );
  }

  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-green-100 to-green-50 text-green-900">

      {/* 🎉 NEW YEAR 2026 COUNTDOWN (MANUAL REMOVE ON JAN 1) */}
      <div className="max-w-7xl mx-auto px-4 pt-8">
  <div className="bg-green-600 rounded-2xl shadow-lg p-6 sm:p-10 text-center text-white mb-8">

    <h1 className="text-3xl sm:text-5xl font-bold mb-4">
      🎉 Happy New Year 2026 🎉
    </h1>

    <p className="text-lg sm:text-xl font-medium mb-6 text-green-100">
      A Big Surprise Is Coming
    </p>

    {/* Countdown Box */}
    <div className="bg-white rounded-xl px-6 py-4 sm:px-10 sm:py-6 inline-block shadow-md">
      <p className="uppercase text-xs tracking-widest text-green-600 mb-2">
        Countdown To 2026
      </p>
      <div className="text-2xl sm:text-4xl font-mono font-bold text-green-700">
        {newYearCountdown}
      </div>
    </div>

    <p className="mt-6 text-sm sm:text-base text-green-100">
      Launching on{" "}
